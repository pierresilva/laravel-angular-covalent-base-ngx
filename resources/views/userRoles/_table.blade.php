<div class="row">
    <div class="col-md-12">
        ...
        @if($userRoles->count())
            ...
            <table class="table table-sm table-striped sp-omit">
              <thead>
                <tr>
                  <th scope="col"><div class="d-flex">
                    @if( method_exists($userRoles, 'appends') )
                  ...
                  <th scope="col">USUARIOS PERMISOS</th>
              ...
              <tbody>
                @foreach($userRoles as $userRole)
                    <tr>
                      <td scope="row"><a href="{{ route('userRoles.show', $userRole->id) }}">{{$userRole->id}}</a></td>
                      <td>{{$userRole->name}}</td>
                      <td>{{$userRole->code}}</td>
                      ...
                      <td>
                          @foreach($userRole->userPermissions as $userPermission)
                                        @if (!$loop->first) , @endif
                                        <a href="{{ route('userPermissions.show', $userPermission->id) }}">{{ $userPermission->name }}(
                                        )</a>
                          @endforeach
                      </td>
                      ...
                      <td class="text-right">
                        <div class="btn-group" role="group">
                            ...
                            <form method="POST" action="{{ route('userRoles.destroy', $userRole->id) }}" accept-charset="UTF-8" style="display: inline;" onsubmit="if(confirm('Delete? Are you sure?')) { return true } else {return false };">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
            ...
            @if( method_exists($userRoles, 'appends') )
              {!! $userRoles->appends(Request::except('page'))->render() !!}
            @endif
        @else
