<div class="row">
    <div class="col-md-12">
        ...
        @if($userPermissionUserRoles->count())

            <table class="table table-sm table-striped sp-omit">
              <thead>
                <tr>
                  <th scope="col"><div class="d-flex">
                    @if( method_exists($userPermissionUserRoles, 'appends') )
              ...
              <tbody>
                @foreach($userPermissionUserRoles as $userPermissionUserRole)
                    <tr>
                      <td scope="row"><a href="{{ route('userPermissionUserRoles.show', $userPermissionUserRole->id) }}">{{$userPermissionUserRole->id}}</a></td>
                      ...
                      <td class="text-right">
                        <div class="btn-group" role="group">
                            ...
                            <form method="POST" action="{{ route('userPermissionUserRoles.destroy', $userPermissionUserRole->id) }}" accept-charset="UTF-8" style="display: inline;" onsubmit="if(confirm('Delete? Are you sure?')) { return true } else {return false };">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
            ...
            @if( method_exists($userPermissionUserRoles, 'appends') )
              {!! $userPermissionUserRoles->appends(Request::except('page'))->render() !!}
            @endif
        @else
